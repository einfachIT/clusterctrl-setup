# Create the bridged controller
  ...
  apt -y install rpiboot bridge-utils screen minicom subversion git libusb-1.0-0-dev nfs-kernel-server busybox $INSTALLEXTRA
...
EOF
  ...
  for ((P=1;P<=252;P++));do
   ...
   mkdir "/var/lib/clusterctrl/nfs/p$P"
  done
